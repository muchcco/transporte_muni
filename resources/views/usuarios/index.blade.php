...
var btnStoreUsuario = () => {
    ...
    $.ajax({
        ...
        dataType: "json",
        cache: false,
        url: "{{ route('usuarios.store_usuario') }}",
        data: formData,
        processData: false,
        contentType: false,
        beforeSend: function () {
...
var btnUpdateUsuario = (id) =>{
    ...
    $.ajax({
        ...
        dataType: "json",
        cache: false,
        url: "{{ route('usuarios.update_usuario') }}",
        data: formData,
        processData: false,
        contentType: false,
        beforeSend: function () {
...
var btnEditPassUsuario = (id) => {
    $.ajax({
        type:'post',
        url: "{{ route('usuarios.modals.md_editpass_usuario') }}",
        dataType: "json",
        data:{"_token": "{{ csrf_token() }}", id : id},
        success:function(data){
            $("#modal_add_em").html(data.html);
...
var update_password_usuario = (id)  => {
    var formData = new FormData();
    ...
    $.ajax({
        ...
        dataType: "json",
        cache: false,
        url: "{{ route('usuarios.update_password_usuario') }}",
        data: formData,
        processData: false,
        contentType: false,
        beforeSend: function () {
...
var EliminarUsuario = (id) => {
    ...
    swal.fire({
        ...
        if (result.value) {
                $.ajax({
                    url: "{{ route('usuarios.delete_usuario') }}",
                    type: 'post',
        ...
        <div id="kt_content_container" class=" container-xxl ">
            <div class="card card-bordered bordered-success ">
                <div class="card-header">
                    <h3 class="card-title">Lista de usuarios al sistema</h3>
                    <div class="card-toolbar">
                        <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#large-Modal" onclick="btnAddusuario()">
                            Registrar Nuevo
                        </button>
                    </div>
                </div>
